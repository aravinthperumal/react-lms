...
export const issueBook = createAsyncThunk(
  ...
  async (data: Omit<BookTransaction, "id">, { dispatch }) => {
    const response = await axios.post<BookTransaction>(
    ...
    await dispatch(
      updateBookCopies({ bookId: data.bookId, change: MINUS_ONE }),
    );
...
export const returnBook = createAsyncThunk(
  ...
  async (data: BookTransaction, { dispatch }) => {
    const response = await axios.put<BookTransaction>(
    ...
    await dispatch(
      updateBookCopies({ bookId: data.bookId, change: NUMBER_ONE }),
    ).unwrap();
    return response.data;
  },
);
...
export const fetchBookTransaction = createAsyncThunk(
  ...
  async (params: Record<string, string>) => {
    const response = await axios.get<BookTransaction[]>(
      `${baseURL}/transactions`,
      {
        params,
      },
