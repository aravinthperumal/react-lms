...
export const checkIfStudentExists = (students: Student[], name: string, email: string): boolean => {
    return students.some((student) => student.name === name && student.email === email);
};
...
export const calculatePenalty = (dueDate: string): number => {
    ...
    const diffTime = convertStringToDate(todayDate()).getTime() - convertStringToDate(dueDate).getTime();
